...
class Filter_RowsNullEmpty(BaseTransformation):
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        if self.is_list:
            return df.filter(pl.col(self.col).is_not_null() & (pl.col(self.col).list.len() > 0))
...
class Create_PopularityScore(BaseTransformation):
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns(
            pl.when(condition).then(norm_calc).otherwise(None).alias("popularity_score")
        )
...
class Create_OneHotFromList(BaseTransformation):
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        unique_vals = (
            df.select(pl.col(self.col).explode().unique()).get_column(self.col).sort().to_list()
        )

        expressions = [
            pl.col(self.col).list.contains(val).cast(pl.Int64).alias(f"{self.col}_{val}")
...
class Create_BGGLink(BaseTransformation):
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        return df.with_columns(
            (pl.lit("https://boardgamegeek.com/boardgame/") + pl.col(self.col).cast(pl.Utf8)).alias(
                "bgg_link"
            )
        )
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        for col_name, type_name in self.columns.items():
            if type_name not in self.TYPE_MAP:
                raise ValueError(
                    f"Unknown type '{type_name}'. Valid types: {list(self.TYPE_MAP.keys())}"
                )
...
class Transform_NormalizeColumn(BaseTransformation):
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        return df.with_columns(
            pl.when(condition).then(norm_calc).otherwise(pl.col(self.col)).alias(self.col)
        )
