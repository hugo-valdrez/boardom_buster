...
# ============================================================================
# EMBEDDING TRANSFORMATIONS - Create embeddings from text columns
# ============================================================================

class Create_DescriptionEmbedding(BaseTransformation):
    """
    Create embeddings from a text column (e.g., game descriptions) using sentence-transformers.
    The embeddings are stored as a list column where each row contains the embedding vector.
    
    Requirements:
        pip install sentence-transformers
    
    Args:
        col: The text column to embed (e.g., 'description')
        output_col: Name for the output embedding column (default: '{col}_embedding')
    ...
    def __init__(
        ...
        self.col = col
        self.output_col = output_col or f"{col}_embedding"
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Get descriptions, replacing nulls with empty strings
        descriptions = df.select(
            pl.col(self.col).fill_null("").alias(self.col)
        ).get_column(self.col).to_list()
        ...
        return df.with_columns(
            pl.Series(name=self.output_col, values=embeddings_list)
        )