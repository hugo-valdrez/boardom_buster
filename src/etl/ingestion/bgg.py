        ...
        for game in response_list:
            ...
            if isinstance(game["name"], list):
                game_data["name"] = next(
                    (
                        name["@value"]
                        for name in game.get("name", [])
                        if name.get("@type", "").lower() == "primary"
                    ),
                    "Unknown - Game name not found",
                )
            else:
                game_data["name"] = game.get("name", {}).get(
                    "@value", "Unknown - Game name not found"
                )
...
async def fetch_bgg_data(
    ...
    try:
        for attempt in range(max_retries):
            ...
            logger.info(
                f"Making request attempt {attempt+1}/{max_retries} \
                for batch {batch_ids.split(',')[0]} to {batch_ids.split(',')[-1]}"
            )
            ...
            logger.info(
                f"Received response for batch {batch_ids.split(',')[0]} to {batch_ids.split(',')[-1]}"
            )

            if response.status_code == 200:
                logger.info(
                    f"Successfully fetched data for batch {batch_ids.split(',')[0]} to {batch_ids.split(',')[-1]}"
                )
                ...
                logger.info(
                    f"Found {len(board_game_items)} board game items out of {len(parsed_response)} total items"
                )
                response_data = prepare_data(response_list=board_game_items)
                logger.info(
                    f"Successfully processed {len(response_data)} games \
                    from batch {batch_ids.split(',')[0]} to {batch_ids.split(',')[-1]}"
                )
        ...
        raise RuntimeError(
            f"Connection timeout when fetching batch {batch_ids.split(',')[0]} to {batch_ids.split(',')[-1]}"
        )
    except httpx.ReadTimeout:
        raise RuntimeError(
            f"Read timeout when fetching batch {batch_ids.split(',')[0]} to {batch_ids.split(',')[-1]}"
        )
    except Exception as e:
        raise RuntimeError(
            f"Unexpected error fetching batch {batch_ids.split(',')[0]} to {batch_ids.split(',')[-1]}: {str(e)}"
        )
...
async def continuous_fetch() -> List[Dict[str, Any]]:
    ...
    logger.info(
        f"Starting continuous fetch with batch_size={batch_size}, async_requests={async_requests}"
    )
    ...
    async with httpx.AsyncClient() as client:
        ...
        while consecutive_failures < max_consecutive_failures:
            try:
                ...
                request_batches = [
                    ",".join(str(start + j) for j in range(batch_size)) for start in batch_starts
                ]

                logger.info(
                    f"Processing {async_requests} batches from ID {batch_starts[0]} to {batch_starts[-1] + batch_size - 1}"
                )
                ...
                if batch_valid_results:
                    if os.path.exists(parquet_path):
                        ...
                        logger.info(
                            f"Created new Parquet file with {batch_valid_results} results: {parquet_path}"
                        )
                    ...
                    logger.warning(
                        f"No valid results found in batch. \
