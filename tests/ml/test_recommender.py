...
def sample_processed_data():
    ...
    return pl.DataFrame({
        ...
        "mechanics": [["Trading"], ["Dice Rolling"], ["Area Control"], ["Hand Management"], ["Voting"]],
...
def sample_knn_results():
    ...
    return pl.DataFrame({
        ...
        "bgg_link": ["https://boardgamegeek.com/boardgame/2",
                     "https://boardgamegeek.com/boardgame/3",
                     "https://boardgamegeek.com/boardgame/4"]
    })
...
class TestBoardGameRecommender:
    ...
    def test_recommend_basic(self, mock_add_comments, sample_processed_data, 
                            sample_knn_results, sample_reranked_results):
        ...
        recommender._knn.get_input_game = Mock(return_value=sample_processed_data.filter(pl.col("id") == "1"))
    ...
    def test_recommend_with_custom_weights(self, sample_processed_data):
        ...
        recommender._knn.get_candidates = Mock(return_value=pl.DataFrame({
            "id": ["2"], "name": ["Game 2"], "cosine_distance": [0.1],
            "publication_year": [2016], "playing_time": [45],
            "avg_rating": [8.0], "bayesian_avg_rating": [8.1],
            "popularity_score": [0.9], "to_recommend": [1],
            "mechanics": [["Dice"]], "categories": [["Family"]],
            "bgg_link": ["https://bgg.com/2"]
        }))
        recommender._knn.get_input_game = Mock(return_value=sample_processed_data.filter(pl.col("id") == "1"))
        recommender._reranker.rerank = Mock(return_value=pl.DataFrame({
            "id": ["2"], "name": ["Game 2"], "final_score": [0.9],
            "cosine_similarity": [0.9], "normalized_year_similarity": [0.8],
            "normalized_playing_time_similarity": [0.85],
            "normalized_avg_rating": [0.95], "normalized_popularity": [0.9]
        }))
